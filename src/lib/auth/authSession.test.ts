...
import { SESSION_RENEW_WINDOW_MS } from "./authTypes";
...
test("getAuthSession returns null and deletes expired session record", async () => {
...
test("getAuthSession does not rotate soon-to-expire session in render-safe flow", async () => {
  const now = Date.now();
  ...
  assert.equal(writeCookie.mock.callCount(), 0);
});
